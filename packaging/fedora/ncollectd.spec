...
%define build_with_multipathd 0%{!?_without_multipathd:1}
...
# Plugins not buildable on RHEL
%if 0%{?rhel}
%define build_with_gps 0%{?_with_gps:1}
%define build_with_nut 0%{?_with_nut:1}
%define build_with_modbus 0%{?_with_modbus:1}
%define build_with_slurm 0%{?_with_slurm:1}
%define build_with_sigrok 0%{?_with_sigrok:1}
%define build_with_xencpu 0%{?_with_xencpu:1}
%define build_with_notify_email 0%{?_with_notify_email:1}
%define build_with_mongodb 0%{?_with_mongodb:1}
%define build_with_write_mongodb 0%{?_with_write_mongodb:1}
%define build_with_mosquitto 0%{?_with_mosquitto:1}
%define build_with_write_mqtt 0%{?_with_write_mqtt:1}
%define build_with_write_amqp1 0%{?_with_write_amqp1:1}
...
%define build_with_sssd 0%{?_with_sssd:1}
...
%define build_with_nfacct 0%{?_with_nfacct:1}
%define build_with_varnish 0%{?_with_varnish:1}
%define build_with_unbound 0%{?_with_unbound:1}
%define build_with_nsd 0%{?_with_nsd:1}
%define build_with_cups 0%{?_with_cups:1}
%define build_with_cert 0%{?_with_cert:1}
%define build_with_certmonger 0%{?_with_certmonger:1}
%define build_with_virt 0%{?_with_virt:1}
%define build_with_docker 0%{?_with_docker:1}
%define build_with_systemd 0%{?_with_systemd:1}
%define build_with_systemd_resolved 0%{?_with_systemd_resolved:1}
%define build_with_sssd 0%{?_with_sssd:1}
%define build_with_logind 0%{?_with_logind:1}
...
%if 0%{?rhel} && 0%{?rhel} == 6
%define cmake         %{cmake3}
%define cmake_build   %{__cmake3} --build . %{?_smp_mflags}
...
BuildRequires: postgresql-devel
%description pgpool
The pgpool lugin collects metrics from a Pgpool-II instance.
%endif
...
%else
%{__install} -Dp -m0755 %{SOURCE2} %{buildroot}%{_initrddir}/ncollectd
%endif
...
%else
# stop ncollectd only when uninstalling
if [ $1 -eq 0 ]; then
    /sbin/service ncollectd stop >/dev/null 2>&1 || :
    /sbin/chkconfig --del ncollectd || :
fi
%endif
...
if [ $1 -eq 1 ]; then
    /sbin/service ncollectd condrestart >/dev/null 2>&1 || :
fi
%endif
