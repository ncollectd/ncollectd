...
DEF_TEST(test01)
{
    ...
    pthread_create(&thread_id, NULL, httpd_thread, "src/plugins/nginx/test01");

    config_item_t ci = (config_item_t) {
        .key = "plugin",
        .values_num = 1,
        .values = (config_value_t[]) {{.type = CONFIG_TYPE_STRING, .value.string ="nginx"}},
        .children_num = 1,
                        ...
                        .values = (config_value_t[]) {
                            {
                                ...
                                .value.string = "http://127.0.0.1:32222/status"
                            },
                        }
                    }
                }
    ...
    EXPECT_EQ_INT(0, plugin_test_do_read(NULL, NULL, &ci, "src/plugins/nginx/test01/expect.txt"));
