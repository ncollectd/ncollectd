...
ncollectd-unixsock - Documentation of ncollectd's C<unixsock plugin>

=head1 SYNOPSIS

  # See ncollectd.conf(5)
  LoadPlugin unixsock
  # ...
  <Plugin unixsock>
    SocketFile "/path/to/socket"
    SocketGroup "ncollectd"
    SocketPerms "0770"
    DeleteSocket false
  </Plugin>
...
the daemon. This can be used to use the values collected by ncollectd in other
applications, such as monitoring solutions, or submit externally collected
values to ncollectd.

For example, this plugin is used by L<ncollectd-nagios(1)> to check if some
...
plugin within ncollectd. I<type> identifies the type and number of values
(i.E<nbsp>e. data-set) passed to ncollectd. A large list of predefined
...
B<ncollectd> ships the Perl-Module L<Collectd::Unixsock> which
...
L<ncollectd(1)>,
L<ncollectd.conf(5)>,
L<ncollectd-nagios(1)>,
L<unix(7)>

=head1 AUTHOR

Florian Forster E<lt>octo@ncollectd.orgE<gt>
