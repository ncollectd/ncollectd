...
static int pass_sym_alloc(struct node *n, void *_pb)
{
    ...
    if (err) {
        if (err == -ENOENT)
            PLUGIN_ERROR("%s: unknown symbol '%s'.", STRNODELOC(n), STRNODELOC(n));
    } else {
        assert(n->sym);
    }
...
static int pass_type_infer(struct node *n, void *_pb)
{
    ...
    if (n->sym->func->type_infer)
        return n->sym->func->type_infer(n->sym->func, n, pb);

    return 0;
}

static int pass_type_report(struct node *n, __attribute__((unused)) void *_pb)
{
    if (!n->sym->type)
        PLUGIN_ERROR("%s: type of symbol '%s' is unknown.", STRNODELOC(n), STRNODE(n));

    return 0;
}

static int pass_type_validate(struct node *n, __attribute__((unused)) void *_pb)
{
    if (!n->sym->type)
        return -EINVAL;
...
static int pass_ir_pre(struct node *n, void *_pb)
{
    ...
    ir_emit_comment(pb->ir, pass_ir_comment(n, ">pre "));
...
static int pass_ir_post(struct node *n, void *_pb)
{
    ...
    ir_emit_comment(pb->ir, pass_ir_comment(n, ">post"));
...
int ply_compile(struct ply *ply)
{
    ...
    for (rewrites = 0; rewrites < 10; rewrites++) {
        ...
        err = err ? err : run_walk(ply, NULL, pass_type_infer);
        err = err ? err : run_walk(ply, NULL, pass_rewrite);
        if (err < 0)
            return err;
    ...
    err = err ? err : run_walk(ply, NULL, pass_sym_alloc);
    err = err ? err : run_walk(ply, NULL, pass_type_infer);

    err = err ? err : run_walk(ply, NULL, pass_type_report);
    err = err ? err : run_walk(ply, NULL, pass_type_validate);

    err = err ? err : run_ir(ply);
    err = err ? err : run_bpf(ply);
