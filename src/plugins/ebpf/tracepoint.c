...
static int tracepoint_data_loc_fprint(__attribute__((unused)) struct type *t, FILE *fp,
                                      const void *_data)
{
    const uint32_t *data = _data;

    return fprintf(fp, "dynamic(%u bytes, offset %#x)", *data >> 16, *data & 0xffff);
}
...
static int tracepoint_dyn_ir_post(__attribute__((unused)) const struct func *func, struct node *n,
                                  struct ply_probe *pb)
{
    struct node *ctx, *arg;
    struct ir *ir = pb->ir;
...
static int tracepoint_dyn_rewrite(__attribute__((unused)) const struct func *func, struct node *n,
                                  __attribute__((unused)) struct ply_probe *pb)
{
    ...
    struct node *ctx = node_expr(&n->loc, "ctx", NULL);
    ctx->up = arg->up;
...
static int tracepoint_dyn_type_infer(__attribute__((unused)) const struct func *func,
                                     struct node *n, struct ply_probe *pb)
{
    ...
    struct node *arg = ctx->next;
    struct node *len = arg->next;

    if (!(arg->sym->type && (!len || len->sym->type)))
        return 0;

    if (arg->sym->type != &t_tracepoint_data_loc) {
        PLUGIN_ERROR("%s: expected a dynamic data pointer (__data_loc), but '%s' is of type '%s.'",
...
static int tracepoint_data_ir_post(__attribute__((unused)) const struct func *func, struct node *n,
                                   __attribute__((unused)) struct ply_probe *pb)
{
    struct node *ctx = n->expr.args;
...
static int tracepoint_data_rewrite(__attribute__((unused)) const struct func *func, struct node *n,
                                   __attribute__((unused)) struct ply_probe *pb)
{
...
static int tracepoint_sym_alloc(struct ply_probe *pb, struct node *n)
{
    ...
    int err = func_static_validate(func, n);
    if (err)
        return err;
...
static int tracepoint_parse_field(char *line, struct tfield *f, size_t *struct_size)
{
    ...
    char *offs_s = strtok_r(NULL, ";", &save);
    char *size_s = strtok_r(NULL, ";", &save);
    char *sign_s = strtok_r(NULL, ";", &save);
    ...
    unsigned long offs = strtoul(offs_s, NULL, 0);
    unsigned long size = strtoul(size_s, NULL, 0);
    unsigned long sign = strtoul(sign_s, NULL, 0);
    if ((offs == ULONG_MAX) || (size == ULONG_MAX) || (sign == ULONG_MAX))
        return -EINVAL;

    char *name = rindex(type, ' ');
    *name++ = '\0';

    char *array = index(name, '[');
    if (array) {
        *array++ = '\0';
    ...
    /* PLUGIN_DEBUG("type:%s name:%s len:%lu offs:%lu size:%lu sign:%lu.", */
    /*    type, name, len, offs, size, sign); */

    struct type *t = tracepoint_parse_type(type, size, sign);
    if (!t)
        return -EINVAL;
...
static int tracepoint_parse(struct ply_probe *pb)
{
    ...
    FILE *fmt = fopenf("r", "%s/format", tp->path);
    if (!fmt)
        return -ENOENT;
    ...
    while (fgets(line, sizeof(line), fmt)) {
        ...
        int err = tracepoint_parse_field(line, &t->sou.fields[n - 1], &t->sou.size);
        if (err < 0) {
            free(t->sou.fields);
            return err;
        }
...
static int tracepoint_attach(struct ply_probe *pb)
{
    ...
    if (tp->evfd < 0) {
        PLUGIN_ERROR("%s: Unabled to attach tracepoint: %s.",
...
{
    struct tracepoint *tp = xcalloc(1, sizeof(*tp));
    pb->provider_data = tp;

    const char *name = strchr(pb->probe, ':');
    assert(name);
    name++;

    size_t pathsz;
    FILE *fp = open_memstream(&tp->path, &pathsz);
    fprintf(fp, TRACEPATH "events/%s", name);
    fclose(fp);

    tp->data_func = tracepoint_data_func;

    struct type *data_t  = xcalloc(1, sizeof(*data_t));
    *data_t = (struct type) {
        .ttype = T_STRUCT,
        .sou = { .name = "data" },
    };

    struct type *datap_t = xcalloc(1, sizeof(*datap_t));
    *datap_t = (struct type) {
        .ttype = T_POINTER,
    ...
    int err = tracepoint_parse(pb);
    if (err) {
        PLUGIN_ERROR("%s: Unable to parse tracepoint at %s.", pb->probe, tp->path);
        goto err_free;
    }
    return 0;
