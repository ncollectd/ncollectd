...
static int kprobe_regs_ir_post(__attribute__((unused)) const struct func *func, struct node *n,
...
static int kprobe_regs_rewrite(__attribute__((unused)) const struct func *func, struct node *n,
                               __attribute__((unused)) struct ply_probe *pb)
{
...
static int kprobe_caller_rewrite(__attribute__((unused)) const struct func *func, struct node *n,
                                 __attribute__((unused)) struct ply_probe *pb)
{
    struct node *new;
    const char *reg;
...
static int kprobe_arg_rewrite(__attribute__((unused)) const struct func *func, struct node *n,
                              __attribute__((unused)) struct ply_probe *pb)
{
    struct node *new;
    const char *reg;
    ...
    if (!reg) {
        PLUGIN_ERROR("%s: the location of %s is unknown.", STRNODELOC(n), STRNODE(n));
...
{
    struct xprobe *xp = xcalloc(1, sizeof(*xp));
    xp->type = 'p';
