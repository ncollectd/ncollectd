...
// SPDX-FileCopyrightText: Copyright (C) Ism Hong
// SPDX-FileContributor: Ism Hong <ism.hong at gmail.com>
...
static int profile_sym_alloc(__attribute__((unused)) struct ply_probe *pb,
...
static int profile_attach(struct ply_probe *pb)
{
    ...
    if (data->cpu != -1) {
        data->evfds[0] = perf_event_attach_profile(pb, data->cpu,
                         data->freq);
        if (data->evfds[0] < 0) {
            PLUGIN_ERROR("%s: Unable to attach profile probe: %s.",
