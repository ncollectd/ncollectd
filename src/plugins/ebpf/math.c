                            ...
                            __attribute__((unused)) struct ply_probe *pb)
{
    ...
    if (type_base(expr->sym->type)->ttype != T_SCALAR) {
        PLUGIN_ERROR("%s: argument of '%s' must be a scalar value, but '%s' is of type '%s'.",
                     STRNODELOC(expr), STRNODE(n), STRNODE(expr), STRTYPE(expr->sym->type));
        return -EINVAL;
    }
...
static int logop_ir_post(const struct func *func, struct node *n, struct ply_probe *pb)
{
    struct bpf_insn jmp;
    int lreg = 0, rreg = 0;
    int start;

    struct node *lval = n->expr.args;
    struct node *rval = lval->next;
     ...
     * end:
     */
    int16_t out = ir_alloc_label(pb->ir);
    if (!strcmp(func->name, "&&")) {
        start = 0;
    ...
    ir_init_irs(pb->ir, &n->sym->irs, n->sym->type);
    int dst = (n->sym->irs.loc == LOC_REG) ? n->sym->irs.reg : BPF_REG_0;
...
static int logop_type_infer(__attribute__((unused)) const struct func *func, struct node *n,
                            __attribute__((unused)) struct ply_probe *pb)
{
    if (n->sym->type)
        return 0;
    ...
    struct type *ltype = lval->sym->type;
    struct type *rtype = rval->sym->type;
    if (!ltype || !rtype)
        return 0;

    if (type_base(ltype)->ttype != T_SCALAR) {
        PLUGIN_ERROR("%s: left side of '%s' must be a scalar value, but '%s' is of type '%s'.",
                     STRNODELOC(n), STRNODE(n), STRNODE(lval), STRTYPE(ltype));
        return -EINVAL;
    }

    if (type_base(rtype)->ttype != T_SCALAR) {
        PLUGIN_ERROR("%s: right side of '%s' must be a scalar value, but '%s' is of type '%s'.",
                     STRNODELOC(n), STRNODE(n), STRNODE(rval), STRTYPE(rtype));
        return -EINVAL;
    }
...
static int relop_type_infer(__attribute__((unused)) const struct func *func, struct node *n,
                            __attribute__((unused)) struct ply_probe *pb)
{
    struct node *lval, *rval;
...
static int binop_type_infer(__attribute__((unused)) const struct func *func, struct node *n,
                            __attribute__((unused)) struct ply_probe *pb)
{
    if (n->sym->type)
        return 0;
