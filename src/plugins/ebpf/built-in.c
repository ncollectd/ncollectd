...
static int num_ir_post(__attribute__((unused)) const struct func *func, struct node *n,
               struct ply_probe *pb)
{
...
{
    size_t len = ((strlen(n->string.data) ? strlen(n->string.data) : 1) + 7) & ~7;

    return type_array_of(&t_char, len);
}

static int string_ir_post(__attribute__((unused)) const struct func *func, struct node *n,
                 struct ply_probe *pb)
{
    struct irstate *irs = &n->sym->irs;
    ...
    ir_emit_data(pb->ir, irs->stack, n->string.data, type_sizeof(n->sym->type));
    return 0;
}
...
static int env_rewrite(__attribute__((unused)) const struct func *func, struct node *n,
               __attribute__((unused)) struct ply_probe *pb)
{
    struct node *new;
    char *end;

    const char *val = getenv(n->expr.func + 1);
    if (!val || !val[0])
        goto string;
    ...
    string:
        new = node_string(&n->loc, strdup(val ? val : ""));
    }
...
static int ident_ir_post(__attribute__((unused)) const struct func *func, struct node *n,
             struct ply_probe *pb)
{
    if (n->sym->type->ttype == T_MAP && !n->sym->irs.hint.lval) {
         ...
         */
        ir_emit_insn(pb->ir, ST_IMM(bpf_width(irs->size), irs->stack, 0), BPF_REG_BP, 0);
    }
...
static int built_in_probe(__attribute__((unused)) struct ply_probe *pb)
{
    return 0;
}
