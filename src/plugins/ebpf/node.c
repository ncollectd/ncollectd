...
char *strnode(struct node *n, char *buf, size_t buflen)
{
    switch (n->ntype) {
    case N_EXPR:
        snprintf(buf, buflen, "%s", n->expr.func);
        break;
    case N_NUM:
        if (n->num.unsignd)
            snprintf(buf, buflen, "%"PRIu64, n->num.u64);
        else
            snprintf(buf, buflen, "%"PRId64, n->num.s64);
        break;
    case N_STRING:
        snprintf(buf, buflen, "\"%s\"", n->string.data);
        break;
    default:
...
{
    return nloc->first_line | nloc->first_column | nloc->last_line | nloc->last_column;
}

char *strnode_loc(struct node *n, char *buf, size_t buflen)
{
    if (!node_nloc_valid(&n->loc))
        return strnode(n, buf, buflen);

    /* TODO: get real filename */
    if (n->loc.first_line != n->loc.last_line)
        snprintf(buf, buflen, "<input>:%d-%d:", n->loc.first_line, n->loc.last_line);
    else
        snprintf(buf, buflen, "<input>:%d:", n->loc.first_line);

    if (n->loc.first_column != n->loc.last_column)
        snprintf(buf, buflen, "<input>:%d-%d", n->loc.first_column, n->loc.last_column);
    else
        snprintf(buf, buflen, "<input>:%d", n->loc.first_column);
...
void node_free(struct node *node)
{
    while(node != NULL) {
        ...
        if (node->ntype == N_STRING) {
            free(node->string.data);
        } else if (node->ntype == N_EXPR) {
            free(node->expr.func);
            node_free(node->expr.args);
        }
...
int node_replace(struct node *n, struct node *new)
{
    ...
    if (new->up && (new->up->ntype == N_EXPR) && (new->up->expr.args == n))
