...
static void strcmp_emit(struct ir *ir, uint16_t dst, ssize_t a, ssize_t b,
                        const char *blit, size_t len)
{
    int literal = blit ? 1 : 0;

    int16_t done = ir_alloc_label(ir);
...
static int strcmp_ir_post(__attribute__((unused)) const struct func *func, struct node *n,
                          struct ply_probe *pb)
{
    int invert = 0;
    const char *blit = NULL;

    struct node *a = n->expr.args;
    ...
    ssize_t len = min(type_sizeof(a->sym->type), type_sizeof(b->sym->type));

    uint16_t dst = sym_in_reg(n->sym) ? n->sym->irs.reg : BPF_REG_0;
...
static int strcmp_type_infer(__attribute__((unused)) const struct func *func, struct node *n,
                             __attribute__((unused)) struct ply_probe *pb)
{
    ...
    for (int i = 0; i < 2; i++) {
        ...
        if (!type_is_string(s[i]->sym->type))
            PLUGIN_WARNING("%s: '%s' is of type '%s', a string was expected.",
                           STRNODELOC(n), STRNODE(s[i]), STRTYPE(s[i]->sym->type));
    }
...
static int str_ir_post(__attribute__((unused)) const struct func *func, struct node *n,
               struct ply_probe *pb)
{
...
static int mem_ir_post(__attribute__((unused)) const struct func *func, struct node *n,
               struct ply_probe *pb)
{
    struct node *ptr = n->expr.args;
...
static int mem_type_infer(__attribute__((unused)) const struct func *func, struct node *n,
                          struct ply_probe *pb)
{
    struct node *arg, *len;
    size_t sz = pb->ply->string_size;
    ...
    if (type_sizeof(arg->sym->type) > (ssize_t)sizeof(void *)) {
        PLUGIN_ERROR("%s: can not cast '%s', of type '%s', to a pointer.",
                     STRNODELOC(n), STRNODE(arg), STRTYPE(arg->sym->type));
        return -EINVAL;
    }

    if (len) {
        if (len->ntype != N_NUM) {
            PLUGIN_ERROR("%s: length must be a constant, but '%s' is of type '%s'.",
                         STRNODELOC(n), STRNODE(len), STRTYPE(len->sym->type));
            return -EINVAL;
        }

        sz = (size_t)len->num.u64;
        if (sz > MAX_BPF_STACK) {
            PLUGIN_ERROR("%s: length is larger than the maximum allowed stack size (%d).",
                         STRNODELOC(n), MAX_BPF_STACK);
            return -EINVAL;
        }
    }
...
static int uptr_ir_post(__attribute__((unused)) const struct func *func, struct node *n,
                        struct ply_probe *pb)
{
...
static int uptr_type_infer(__attribute__((unused)) const struct func *func, struct node *n,
                           __attribute__((unused)) struct ply_probe *pb)
{
...
static int struct_deref_rewrite(__attribute__((unused)) const struct func *func, struct node *n,
                                __attribute__((unused)) struct ply_probe *pb)
{
    struct node *sou = n->expr.args;
    struct node *member = sou->next;
    
    /* sou->member => (*sou).member */
    struct node *new = node_expr(&n->loc, ".",
                                 node_expr(&n->loc, "u*",
                                           node_expr_ident(&sou->loc, sou->expr.func), NULL),
...
static int struct_deref_type_infer(__attribute__((unused)) const struct func *func, struct node *n,
                                   __attribute__((unused)) struct ply_probe *pb)
{
    if (n->sym->type)
        return 0;
    ...
    struct type *t = type_base(sou->sym->type);

    if (t->ttype != T_POINTER) {
        PLUGIN_ERROR("%s: %s is not a pointer (type '%s').", STRNODELOC(n),
                     STRNODE(sou), STRTYPE(sou->sym->type));
        return -EINVAL;
    }
    ...
    /* TODO: add union */
    if (t->ttype != T_STRUCT) {
        PLUGIN_ERROR("%s: %s is neither struct nor union (type '%s').", STRNODELOC(n),
                     STRNODE(sou), STRTYPE(sou->sym->type));
        return -EINVAL;
    }

    struct tfield *f = tfields_get(t->sou.fields, member->string.data);
    if (!f) {
        PLUGIN_ERROR("%s: type '%s' has no member named %s.",
                     STRNODELOC(n), STRTYPE(t), STRNODE(member));
        return -EINVAL;
    }
...
static int struct_dot_ir_pre(__attribute__((unused)) const struct func *func, struct node *n,
                __attribute__((unused)) struct ply_probe *pb)
{
    struct node *sou = n->expr.args;
...
static int struct_dot_ir_post(__attribute__((unused)) const struct func *func, struct node *n,
                struct ply_probe *pb)
{
    struct node *sou = n->expr.args;
    struct node *member = sou->next;
    struct irstate *dst = &n->sym->irs;
    
    ir_init_sym(pb->ir, n->sym);

    ssize_t offset = type_offsetof(type_base(sou->sym->type), member->string.data);
    assert(offset >= 0);
...
static int struct_dot_type_infer(__attribute__((unused)) const struct func *func, struct node *n,
                                 __attribute__((unused)) struct ply_probe *pb)
{
    if (n->sym->type)
        return 0;
    ...
    /* TODO: add union */
    if (t->ttype != T_STRUCT) {
        PLUGIN_ERROR("%s: %s is neither struct nor union (type '%s').",
                     STRNODELOC(n), STRNODE(sou), STRTYPE(sou->sym->type));
        return -EINVAL;
    }
...
static int deref_ir_post(__attribute__((unused)) const struct func *func, struct node *n,
                         struct ply_probe *pb)
{
    struct node *ptr = n->expr.args;
...
static int deref_type_infer(__attribute__((unused)) const struct func *func, struct node *n,
                            __attribute__((unused)) struct ply_probe *pb)
{
    struct node *ptr = n->expr.args;
    struct type *t;
    ...
    if (t->ttype != T_POINTER) {
        PLUGIN_ERROR("%s: can't dereference %s (type '%s').",
                     STRNODELOC(n), STRNODE(ptr), STRTYPE(ptr->sym->type));
        return -EINVAL;
    }
...
static int struct_ir_pre(__attribute__((unused)) const struct func *func, struct node *n,
                         struct ply_probe *pb)
{
...
static int struct_ir_post(__attribute__((unused)) const struct func *func, struct node *n,
                          struct ply_probe *pb)
{
    struct type *t = type_base(n->sym->type);
    ssize_t stack = n->sym->irs.stack;
...
static int struct_type_infer(__attribute__((unused)) const struct func *func, struct node *n,
                             __attribute__((unused)) struct ply_probe *pb)
{
    struct node *arg;
    struct tfield *f;
    ...
    struct tfield *fields = xcalloc(nargs + 1, sizeof(*fields));

    for (arg = n->expr.args, f = fields, i = 0; arg; arg = arg->next, f++, i++) {
        asprintf(&f->name, "f%d", i);
        f->type = arg->sym->type;
    }

    asprintf(&t->sou.name, ":anon_%p", (void *)n);
    t->ttype = T_STRUCT;
    t->sou.fields = fields;
...
static int map_ir_post(__attribute__((unused)) const struct func *func, struct node *n,
               struct ply_probe *pb)
{
    struct node *map = n->expr.args;
    struct node *key = map->next;
    ssize_t stack = key->sym->irs.stack;
    ...
    int16_t lmiss = ir_alloc_label(pb->ir);
    int16_t lhit  = ir_alloc_label(pb->ir);
...
static int map_type_validate(__attribute__((unused)) struct node *n)
{
    /* TODO */
    return 0;
}

static int map_type_infer(__attribute__((unused)) const struct func *func, struct node *n,
                          __attribute__((unused)) struct ply_probe *pb)
{
    struct node *map = n->expr.args;
    struct node *key = map->next;
    ...
    if (map->sym->type) {
        if (map->sym->type->ttype != T_MAP) {
            PLUGIN_ERROR("%s: can't lookup a key in %s (type '%s'), which is not a map.",
                         STRNODELOC(n), STRNODE(map), STRTYPE(map->sym->type));
            return -EINVAL;
        }
    ...
    map->sym->type = type_map_of(key->sym->type, n->sym->type, BPF_MAP_TYPE_HASH, 0);
    return 0;
}

static int map_static_validate(__attribute__((unused)) const struct func *func, struct node *n)
{
    if (n->expr.args->ntype != N_EXPR || !n->expr.args->expr.ident) {
        PLUGIN_ERROR("%s: can't lookup a key in %s, which is not a map.",
...
static int assign_ir_pre(__attribute__((unused)) const struct func *func, struct node *n,
                struct ply_probe *pb)
{
...
static int assign_ir_post(__attribute__((unused)) const struct func *func, struct node *n,
                 struct ply_probe *pb)
{
    struct node *lval, *rval;
...
static int assign_type_infer(__attribute__((unused)) const struct func *func, struct node *n,
                             struct ply_probe *pb)
{
    struct node *lval, *rval;
    int err;
    ...
    if (!lval->sym->type) {
        ...
        err = map_type_infer(lval->sym->func, lval, pb);
        if (err)
            return err;
    }
    ...
    PLUGIN_ERROR("%s: can't assign %s (type '%s'), to %s (type '%s').",
                 STRNODELOC(n), STRNODE(rval), STRTYPE(rval->sym->type),
                 STRNODE(lval), STRTYPE(lval->sym->type));

    return -EINVAL;
}

static int assign_static_validate(__attribute__((unused)) const struct func *func, struct node *n)
{
    struct node *lval;
    ...
    PLUGIN_ERROR("%s: can't assign a value to %s.", STRNODELOC(n), STRNODE(lval));
...
static int agg_ir_post(__attribute__((unused)) const struct func *func, struct node *n,
                         struct ply_probe *pb)
{
...
static int delete_ir_pre(__attribute__((unused)) const struct func *func, struct node *n,
                   __attribute__((unused)) struct ply_probe *pb)
{
    struct node *arg;
...
static int delete_ir_post(__attribute__((unused)) const struct func *func, struct node *n,
                 struct ply_probe *pb)
{
    struct node *map, *key;
...
static int delete_static_validate(__attribute__((unused)) const struct func *func, struct node *n)
{
    struct node *arg;
    ...
    PLUGIN_ERROR("%s: can't delete %s, a map was expected.", STRNODELOC(n), STRNODE(arg));
    return -EINVAL;
}
...
static struct ply_return clear_ev_handler(__attribute__((unused)) struct buffer_ev *ev, void *_n)
{
...
static int clear_type_infer(__attribute__((unused)) const struct func *func, struct node *n,
                            __attribute__((unused)) struct ply_probe *pb)
{
...
static int clear_rewrite(__attribute__((unused)) const struct func *func, struct node *n,
                         struct ply_probe *pb)
{
...
static int clear_static_validate(__attribute__((unused)) const struct func *func, struct node *n)
{
    struct node *arg = n->expr.args;
