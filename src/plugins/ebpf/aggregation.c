...
static int count_ir_post(__attribute__((unused)) const struct func *func, struct node *n,
                         struct ply_probe *pb)
{
...
static int sum_ir_post(__attribute__((unused)) const struct func *func, struct node *n,
                       struct ply_probe *pb)
{
    struct node *mapop = n->up->expr.args;
...
static int sum_type_infer(__attribute__((unused)) const struct func *func, struct node *n,
                          __attribute__((unused)) struct ply_probe *pb)
{
    ...
    struct type *t = type_base(arg->sym->type);
    if (t->ttype != T_SCALAR) {
        PLUGIN_ERROR("%s: can't sum non-scalar value %s (type '%s').",
...
static void __quantize_fprint_hist_unicode(FILE *fp, unsigned int count, uint64_t total)
{
    static const unsigned char bar_open[] = { 0xe2, 0x94, 0xa4 };
    static const unsigned char bar_close[] = { 0xe2, 0x94, 0x82 };

    int w = (((float)count / (float)total) * 256.0) + 0.5;
    int space = 32 - ((w +  7) >> 3);
    unsigned char block[] = { 0xe2, 0x96, 0x88 };
...
static int __quantize_fprint_bucket_ext(struct type *t, FILE *fp, int i)
{
    ...
    int64_t slo = i ? (1LL  << i) : 0;
    int64_t ulo = i ? (1ULL << i) : 0;

    uint64_t shi = (1LL  << (i + 1)) - 1;
    uint64_t uhi = (1ULL << (i + 1)) - 1;
...
static int __quantize_fprint_bucket(struct type *t, FILE *fp, int i)
{
    ...
    int lo = __quantize_normalize(i    , &ls);
    int hi = __quantize_normalize(i + 1, &hs);
...
static int quantize_ir_post(__attribute__((unused)) const struct func *func, struct node *n,
                            struct ply_probe *pb)
{
    struct node *mapop = n->up->expr.args;
    struct node *arg = n->expr.args;
...
static int quantize_type_infer(__attribute__((unused)) const struct func *func, struct node *n,
                               __attribute__((unused)) struct ply_probe *pb)
{
    struct node *arg = n->expr.args;
    ...
    struct type *t = type_base(arg->sym->type);
    if (t->ttype != T_SCALAR) {
        PLUGIN_ERROR("%s, can't quantize non-scalar value %s (type '%s').",
                     STRNODELOC(n), STRNODE(arg), STRTYPE(arg->sym->type));
        return -EINVAL;    
    }

    struct type *array = type_array_of(&t_uint, type_sizeof(t) * 8);
