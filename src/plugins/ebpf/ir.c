...
{
    if (reg & vreg_base) {
        sprintf(name, "v%u", (unsigned int)(reg & ~vreg_base));        
...
static void reg_dump(uint16_t reg, int16_t off, FILE *fp)
{
    ...
    if (off < 0)
        fprintf(fp, "[%s - 0x%x]", name, (unsigned int)(-off));
    else if (off > 0)
        fprintf(fp, "[%s + 0x%x]", name, (unsigned int)off);
    else
        fprintf(fp, "%s", name);
            ...
            FILE *fp)
{
    ...
    imm_src:
        fprintf(fp, "#%s0x%x", insn.imm < 0 ? "-" : "",
            (unsigned int)(insn.imm < 0 ? -insn.imm : insn.imm));
        break;
    case BPF_X:
    reg_src:
...
void ir_emit_comment(struct ir *ir, char *comment)
{
    struct vinsn vi;
...
{
    ir_init_irs(ir, &sym->irs, sym->type);
}
...
static void ir_bpf_vreg_replace(struct ir *ir, struct vinsn *last, uint16_t vreg, int reg)
{
    struct vinsn *vi;

    PLUGIN_DEBUG("ir_bpf_generate: v%d -> r%d.", vreg & ~vreg_base, reg);
...
static int ir_bpf_registerize_one(struct ir *ir, struct vinsn *last, uint16_t vreg)
{
    uint16_t clean = 0x3ff;

    for (struct vinsn *vi = ir->vi; clean && (vi < last); vi++) {
        if (vi->vitype != VI_INSN)
            continue;
...
static int ir_bpf_registerize(struct ir *ir)
{
    ...
    for (struct vinsn *vi = &ir->vi[ir->len - 1]; vi >= ir->vi; vi--) {
        if (vi->vitype != VI_INSN)
            continue;

        if (vi->insn.dst & vreg_base) {
            int err = ir_bpf_registerize_one(ir, vi, vi->insn.dst);
            if (err)
                return err;
        }

        if (vi->insn.src & vreg_base) {
            int err = ir_bpf_registerize_one(ir, vi, vi->insn.src);
            if (err)
                return err;
        }
    }
...
static int ir_bpf_jmp_resolve_one(struct ir *ir, struct vinsn *jmp)
{
    ...
    for (struct vinsn *vi = jmp + 1; vi <= &ir->vi[ir->len - 1]; vi++) {
        switch (vi->vitype) {
        case VI_INSN:
        case VI_LDMAP:
            off++;
...
{
    for (struct vinsn *vi = ir->vi; vi <= &ir->vi[ir->len - 1]; vi++) {
        if (vi->vitype != VI_INSN)
            continue;

        if ((BPF_CLASS(vi->insn.bpf.code) == BPF_JMP)
            && (vi->insn.bpf.off < 0)) {
            int err = ir_bpf_jmp_resolve_one(ir, vi);
            if (err)
                return err;
        }
    }
