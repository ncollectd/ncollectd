...
int printf_xfprintxf(__attribute__((unused)) struct printxf *pxf,
             FILE *fp, const char *spec, void *_pd)
{
    ...
    memcpy(val, (uint8_t *)pd->data + type_offsetof(pd->t, pd->f->name), size);
...
static int printf_rewrite(__attribute__((unused)) const struct func *func, struct node *n,
...
static int printf_type_infer(__attribute__((unused)) const struct func *func, struct node *n,
                             __attribute__((unused)) struct ply_probe *pb)
{
    struct printf_evh *pevh;
    ...
    if (!n->expr.args) {
        PLUGIN_ERROR("%s: format specifier missing.", STRNODELOC(n));
        return -EINVAL;
    }

    if (n->expr.args->ntype != N_STRING) {
        PLUGIN_ERROR("%s: format specifier must be a string literal.", STRNODELOC(n));
        return -EINVAL;
    }
...
static int print_rewrite(__attribute__((unused)) const struct func *func, struct node *n,
            struct ply_probe *pb)
{
    struct node *bwrite, *exprs, *ev;
    struct buffer_evh *evh;
...
static int print_type_infer(__attribute__((unused))const struct func *func, struct node *n,
                            __attribute__((unused)) struct ply_probe *pb)
{
    struct buffer_evh *evh;
