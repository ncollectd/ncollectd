...
static pinghost_t *ping_receive_ipv4 (pingobj_t *obj, char *buffer, size_t buffer_len)
{
    ...
    if (icmp_hdr->icmp_type != ICMP_ECHOREPLY) {
        PLUGIN_DEBUG("Unexpected ICMP type: %"PRIu8"\n", icmp_hdr->icmp_type);
        return NULL;
    }
    ...
    for (ptr = obj->table[ident % PING_TABLE_LEN]; ptr != NULL; ptr = ptr->table_next) {
        PLUGIN_DEBUG("hostname = %s, ident = 0x%04x, seq = %i\n",
                     ptr->hostname, (unsigned int)ptr->ident, ((ptr->sequence - 1) & 0xFFFF));
        ...
        PLUGIN_DEBUG("Match found: hostname = %s, ident = 0x%04"PRIx16", seq = %"PRIu16"\n",
    ...
    if (ptr == NULL) {
        PLUGIN_DEBUG("No match found for ident = 0x%04"PRIx16", seq = %"PRIu16"\n", ident, seq);
    }
...
static pinghost_t *ping_receive_ipv6 (pingobj_t *obj, char *buffer, size_t buffer_len)
{
    ...
    if (icmp_hdr->icmp6_type != ICMP6_ECHO_REPLY) {
        PLUGIN_DEBUG("Unexpected ICMP type: %02x\n", icmp_hdr->icmp6_type);
        return NULL;
    }

    if (icmp_hdr->icmp6_code != 0) {
        PLUGIN_DEBUG("Unexpected ICMP code: %02x\n", icmp_hdr->icmp6_code);
        return NULL;
    }
...
static int ping_receive_one (pingobj_t *obj, struct timeval *now, int addrfam)
{
    ...
    if (payload_buffer_len < 0)
    {
        ...
        PLUGIN_DEBUG("recvfrom: %s\n", sstrerror(errno, errbuf, sizeof (errbuf)));
#endif
        return -1;
    }
    PLUGIN_DEBUG("Read %zi bytes from fd = %i\n", payload_buffer_len, fd);
    ...
    for (cmsg = CMSG_FIRSTHDR(&msghdr); cmsg != NULL; cmsg = CMSG_NXTHDR(&msghdr, cmsg)) {
        if (cmsg->cmsg_level == SOL_SOCKET) {
            ...
            if (cmsg->cmsg_type == IP_TOS) {
                memcpy(&recv_qos, CMSG_DATA (cmsg), sizeof (recv_qos));
                PLUGIN_DEBUG("TOSv4 = 0x%02"PRIx8";\n", recv_qos);
            } else if (cmsg->cmsg_type == IP_TTL) {
                memcpy(&recv_ttl, CMSG_DATA (cmsg), sizeof (recv_ttl));
                PLUGIN_DEBUG("TTLv4 = %i;\n", recv_ttl);
            } else {
                PLUGIN_DEBUG("Not handling option %i.\n", cmsg->cmsg_type);
            }
            ...
            if (cmsg->cmsg_type == IPV6_TCLASS) {
                memcpy(&recv_qos, CMSG_DATA (cmsg), sizeof (recv_qos));
                PLUGIN_DEBUG("TOSv6 = 0x%02"PRIx8";\n", recv_qos);
            } else
#ifdef IPV6_HOPLIMIT
            if (cmsg->cmsg_type == IPV6_HOPLIMIT) {
                memcpy(&recv_ttl, CMSG_DATA (cmsg), sizeof (recv_ttl));
                PLUGIN_DEBUG("TTLv6 = %i;\n", recv_ttl);
            } else
#endif
            ...
            PLUGIN_DEBUG("Don't know how to handle unknown protocol %i.\n", cmsg->cmsg_level);
        }
    }
    ...
    if (addrfam == AF_INET) {
        ...
        PLUGIN_DEBUG("ping_receive_one: Unknown address family %i.\n", addrfam);
        return -1;
    }

    PLUGIN_DEBUG("rcvd: %12i.%06i\n", (int)pkt_now.tv_sec, (int)pkt_now.tv_usec);
    PLUGIN_DEBUG("sent: %12i.%06i\n", (int)host->timer->tv_sec, (int)host->timer->tv_usec);
 ...
 * `-> ping_send_one_ipv6                                                    *
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
static ssize_t ping_sendto (pingobj_t *obj, pinghost_t *ph, const void *buf, size_t buflen, int fd)
{
    ssize_t ret;
    ...
    ret = sendto(fd, buf, buflen, 0, (struct sockaddr *)ph->addr, ph->addrlen);
...
static int ping_send_one_ipv4 (pingobj_t *obj, pinghost_t *ph, int fd)
{
    ...
    *icmp4 = (struct icmp) {
    ...
    PLUGIN_DEBUG("Sending ICMPv4 package with ID 0x%04x\n", (unsigned int)ph->ident);
...
static int ping_send_one_ipv6 (pingobj_t *obj, pinghost_t *ph, int fd)
{
    ...
    *icmp6 = (struct icmp6_hdr) {
    ...
    PLUGIN_DEBUG("Sending ICMPv6 package with ID 0x%04x\n", (unsigned int)ph->ident);
...
#ifdef NCOLLECTD_DEBUG
        char errbuf[PING_ERRMSG_LEN];
        PLUGIN_DEBUG("gettimeofday: %s\n", sstrerror(errno, errbuf, sizeof (errbuf)));
#endif
        timerclear (ptr->timer);
        return -1;
    } else {
        PLUGIN_DEBUG("timer set for hostname = %s\n", ptr->hostname);
    }

    if (ptr->addrfamily == AF_INET6) {
        PLUGIN_DEBUG("Sending ICMPv6 echo request to `%s'\n", ptr->hostname);
        if (ping_send_one_ipv6(obj, ptr, fd) != 0) {
            timerclear(ptr->timer);
            return -1;
        }
    } else if (ptr->addrfamily == AF_INET) {
        PLUGIN_DEBUG("Sending ICMPv4 echo request to `%s'\n", ptr->hostname);
        if (ping_send_one_ipv4(obj, ptr, fd) != 0) {
            timerclear(ptr->timer);
            return -1;
        }
    } else {
        PLUGIN_DEBUG("Unknown address family: %i\n", ptr->addrfamily);
        timerclear(ptr->timer);
        return -1;
    }
...
static int ping_set_ttl (pingobj_t *obj, int ttl)
{
    ...
    if (obj->fd4 != -1) {
        if (setsockopt(obj->fd4, IPPROTO_IP, IP_TTL, &ttl, sizeof (ttl))) {
            ret = errno;
            ping_set_error(obj, "ping_set_ttl", sstrerror(ret, errbuf, sizeof (errbuf)));
            PLUGIN_DEBUG("Setting TTLv4 failed: %s\n", errbuf);
        }
    }

    if (obj->fd6 != -1) {
        PLUGIN_DEBUG ("Setting TTLv6 to %i\n", ttl);
        if (setsockopt(obj->fd6, IPPROTO_IPV6, IPV6_UNICAST_HOPS, &ttl, sizeof (ttl))) {
            ret = errno;
            ping_set_error (obj, "ping_set_ttl", sstrerror(ret, errbuf, sizeof (errbuf)));
            PLUGIN_DEBUG("Setting TTLv6 failed: %s\n", errbuf);
        }
    }
...
static int ping_set_qos (pingobj_t *obj, uint8_t qos)
{
    ...
    if (obj->fd4 != -1) {
        PLUGIN_DEBUG ("Setting TP_TOS to %#04"PRIx8"\n", qos);
        if (setsockopt(obj->fd4, IPPROTO_IP, IP_TOS, &qos, sizeof (qos))) {
            ret = errno;
            ping_set_error(obj, "ping_set_qos", sstrerror(ret, errbuf, sizeof (errbuf)));
            PLUGIN_DEBUG("Setting TP_TOS failed: %s\n", errbuf);
        }
    }
    ...
    if (obj->fd6 != -1) {
        ...
        PLUGIN_DEBUG("Setting IPV6_TCLASS to %#04"PRIx8" (%i)\n", qos, tmp);
        if (setsockopt(obj->fd6, IPPROTO_IPV6, IPV6_TCLASS, &tmp, sizeof (tmp))) {
            ret = errno;
            ping_set_error(obj, "ping_set_qos", sstrerror(ret, errbuf, sizeof (errbuf)));
            PLUGIN_DEBUG("Setting IPV6_TCLASS failed: %s\n", errbuf);
        }
    }
...
static int ping_get_ident (void)
{
    ...
    if (did_seed == 0) {
        if ((fd = open("/dev/urandom", O_RDONLY)) != -1) {
            ...
            char errbuf[PING_ERRMSG_LEN];
            PLUGIN_DEBUG("open (/dev/urandom): %s\n", sstrerror(errno, errbuf, sizeof (errbuf)));
        }
#endif
    }
...
static int ping_open_socket(pingobj_t *obj, int addrfam)
{
...
#ifdef NCOLLECTD_DEBUG
        char errbuf[PING_ERRMSG_LEN];
        PLUGIN_DEBUG("socket: %s\n", sstrerror(errno, errbuf, sizeof (errbuf)));
#endif
        return -1;
    } else if (fd >= FD_SETSIZE) {
        ping_set_errno(obj, EMFILE);
        PLUGIN_DEBUG("socket(2) returned file descriptor %d, which is above the file "
    ...
    if (obj->device != NULL) {
        if (setsockopt(fd, SOL_SOCKET, SO_BINDTODEVICE,
            ...
            ping_set_errno(obj, errno);
            PLUGIN_DEBUG("setsockopt (SO_BINDTODEVICE): %s\n", STRERRNO);
            close(fd);
            return -1;
        }
    }
...
#ifdef SO_MARK
    if (obj->set_mark) {
        if (setsockopt(fd, SOL_SOCKET, SO_MARK, &obj->mark, sizeof(obj->mark)) != 0) {
            ping_set_errno(obj, errno);
            PLUGIN_DEBUG("setsockopt (SO_MARK): %s\n", STRERRNO);
            close(fd);
            return -1;
        }
    }
#endif
#ifdef SO_TIMESTAMP
    status = setsockopt(fd, SOL_SOCKET, SO_TIMESTAMP, &(int){1}, sizeof(int));
    if (status != 0) {
        ping_set_errno(obj, errno);
        PLUGIN_DEBUG("setsockopt (SO_TIMESTAMP): %s\n", STRERRNO);
        close(fd);
        return -1;
    }
#endif
    ...
    if (addrfam == AF_INET) {
        ...
        status = setsockopt(fd, IPPROTO_IP, IP_RECVTOS, &(int){1}, sizeof(int));
        if (status != 0) {
            ping_set_errno(obj, errno);
            PLUGIN_DEBUG("setockopt (IP_RECVTOS) failed: %s", STRERRNO);
            close(fd);
            return -1;
        }
#endif
        /* Enable receiving the TTL field */
        status = setsockopt(fd, IPPROTO_IP, IP_RECVTTL, &(int){1}, sizeof(int));
        if (status != 0) {
            ping_set_errno(obj, errno);
            PLUGIN_DEBUG("setockopt (IPPROTO_IP) failed: %s", STRERRNO);
            close(fd);
            return -1;
        }
    }
...
# if defined(IPV6_RECVHOPLIMIT)
        ...
        status = setsockopt(fd, IPPROTO_IPV6, IPV6_RECVHOPLIMIT, &(int){1}, sizeof(int));
        if (status != 0) {
            ping_set_errno(obj, errno);
            PLUGIN_DEBUG("setockopt (IPV6_RECVHOPLIMIT) failed: %s", STRERRNO);
            close(fd);
            return -1;
        }
# endif
...
# if defined(IPV6_RECVTCLASS)
        ...
        status = setsockopt(fd, IPPROTO_IPV6, IPV6_RECVTCLASS, &(int){1}, sizeof(int));
        if (status != 0) {
            ping_set_errno(obj, errno);
            PLUGIN_DEBUG("setockopt (IPV6_RECVTCLASS) failed: %s", STRERRNO);
            close(fd);
            return -1;
        }
# endif
...
int ping_setopt (pingobj_t *obj, int option, void *value)
{
    ...
    case PING_OPT_SOURCE: {
...
#ifdef NCOLLECTD_DEBUG
        if (obj->addrfamily != AF_UNSPEC) {
            PLUGIN_DEBUG("Resetting obj->addrfamily to AF_UNSPEC.\n");
        }
#endif
        ...
        if (ai_list->ai_next != NULL) {
            PLUGIN_DEBUG ("hostname = `%s' is ambiguous.\n", hostname);
        }
#endif
        if (obj->srcaddr == NULL) {
            obj->srcaddrlen = 0;
        ...
        memcpy((void *) obj->srcaddr, (const void *)ai_list->ai_addr, ai_list->ai_addrlen);
...
int ping_send (pingobj_t *obj)
{
    ...
    PLUGIN_DEBUG("Set timeout to %i.%06i seconds\n", (int)timeout.tv_sec, (int)timeout.tv_usec);
    ...
    while (pings_in_flight > 0 || host_to_ping != NULL) {
        ...
        PLUGIN_DEBUG ("Waiting on %i sockets for %u.%06u seconds\n",
        ...
        if (status == -1) {
            ...
            PLUGIN_DEBUG("select: %s\n", obj->errmsg);
            return -1;
        } else if (status == 0) {
            PLUGIN_DEBUG("select timed out\n");
...
int ping_host_add (pingobj_t *obj, const char *host)
{
    ...
    if ((ai_return = getaddrinfo(host, NULL, &ai_hints, &ai_list)) != 0) {
        ...
        char errbuf[PING_ERRMSG_LEN];
#endif
        PLUGIN_DEBUG("getaddrinfo failed\n");
        ping_set_error(obj, "getaddrinfo",
#if defined(EAI_SYSTEM)
    ...
    for (ai_ptr = ai_list; ai_ptr != NULL; ai_ptr = ai_ptr->ai_next) {
        ...
        if ((ai_ptr->ai_canonname != NULL) && (strcmp(ph->hostname, ai_ptr->ai_canonname) != 0)) {
            ...
            PLUGIN_DEBUG("ph->hostname = %s; ai_ptr->ai_canonname = %s;\n",
